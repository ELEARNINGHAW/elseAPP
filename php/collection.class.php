...
function Collection(  $CONFIG, $SQL, $renderer, $util  )
{
   $this->CFG        = $CONFIG;
   $this->sql        = $SQL;
...
#  $this->CFG->C->deb( $x );
  ...
  else
  {
   ...
   $IC[ 'currentSemester' ]= array_pop( $conf[ 'semester' ] ); # aktuelle Semester ist an erster Stelle des Arrays
   
   foreach ( $conf[ 'semester' ]  as $sem )
   {
     $IC[ 'newCollection_id' ] = str_replace( $sem , $IC[ 'currentSemester' ] , $IC[ 'collection_id' ], $cnt);
     $IC[ 'newTitle'         ] = str_replace( $sem , $IC[ 'currentSemester' ] , $IC[ 'title'         ], $cnt);
     $IC[ 'newTitle_short'   ] = str_replace( $sem , $IC[ 'currentSemester' ] , $IC[ 'title_short'   ], $cnt);
     if ( $cnt ) break;
    } 
