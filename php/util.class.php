...
####################### --- TOOLS --- #######################
  ...
  function getGET_EMIL_Values ( )
  {
    ...
    {
      if ( isset ( $_GET[ 'm' ] ) )   { $IDMuser[ 'mail' ] = rawurldecode ( base64_decode ( $_GET[ 'm' ] ) ) ;         }
      else                            { echo "<br>ERROR: no 'mail'             " ;                                     }
      
      if ( isset ( $_GET[ 'fn' ] ) )  { $IDMuser[ 'vorname' ] = rawurldecode ( base64_decode ( $_GET[ 'fn' ] ) );      }
      else                            { echo "<br>ERROR: no 'vorname'          " ;                                     }

      if ( isset ( $_GET[ 'ln' ] ) )  { $IDMuser[ 'nachname' ] = rawurldecode ( base64_decode ( $_GET[ 'ln' ] ) ) ;    }
      else                            { echo "<br>ERROR: no 'nachname'         " ;                                     }
      
      if ( isset ( $_GET[ 'u' ] ) )   { $IDMuser[ 'akennung' ] = rawurldecode ( base64_decode ( $_GET[ 'u' ] ) ) ;     }
      else                            { echo "<br>ERROR: no 'akennung'         " ;                                     }
      
      if ( isset ( $_GET[ 'id' ] ) )  { $IDMuser[ 'matrikelnr' ] = rawurldecode ( base64_decode ( $_GET[ 'id' ] ) ) ;  }
      else                            { echo "<br>ERROR: no 'matrikelnr'       " ;                                     }

      if ( isset ( $_GET[ 'fa' ] ) )  { $IDMuser[ 'fakultaet' ] = rawurldecode ( base64_decode ( $_GET[ 'fa' ] ) ) ;   }
      else                            { echo "<br>ERROR: no 'studiengang'      " ;                                     }
      
      if ( isset ( $_GET[ 'dp' ] ) )  { $IDMuser[ 'department' ] = rawurldecode ( base64_decode ( $_GET[ 'dp' ] ) ) ;  }
      else                            { echo "<br>ERROR: no 'department'       " ;                                     }
      
      if ( isset ( $_GET[ 'sx' ] ) )  { $IDMuser[ 'sex' ] = rawurldecode ( base64_decode ( $_GET[ 'sx' ] ) ) ;         }
      else                            { echo "<br>ERROR: no 'sex'              " ;                                     }

      if ( isset ( $_GET[ 'ro' ] ) )  { $IDMuser[ 'role' ] = rawurldecode ( base64_decode ( $_GET[ 'ro' ] ) ) ;  $IDMuser[ 'role_encode' ] =  $_GET[ 'ro' ] ;       }
      else                            { echo "<br>ERROR: no 'role'             " ;                                     }

      if ( isset ( $_GET[ 'sn' ] ) )  { $Course[ 'shortname' ] = rawurldecode ( base64_decode ( $_GET[ 'sn' ] ) ) ;    }
      else                            { echo "<br>ERROR: no 'course shortname' " ;                                     }
      
      if ( isset ( $_GET[ 'cn' ] ) )  { $Course[ 'fullname' ] = rawurldecode ( base64_decode ( $_GET[ 'cn' ] ) ) ;     }
      else                            { echo "<br>ERROR: no 'course fullname'  " ;                                     }
 ...
 # 
      ...
      {#deb($IDMuser,1);
        $this -> initUpdateUser ( $IDMuser ) ;                                      # deb($Course[  'shortname'   ]);
      ...
      {                                                                             ## echo "--Semesterapparat existiert bereits--";  
        ...
        $IC[ 'location_id'   ] = $this -> resolveLocationID ( $IDMuser[ 'department' ] ) ;
  ...
  {
    if    ( $this->sql -> doUserExist ( $IDMuser[ 'akennung' ] ) ) { $this->sql -> updateUser ( $IDMuser ) ; }  # echo "- Bestehender USER (UPDATE DB )-";
    else                                                           { $this->sql -> initUser   ( $IDMuser ) ; }  # echo "- NEUER USER (INIT DB )-";

    $ans = $this->sql -> getUserAK ( $IDMuser[ 'akennung' ] ) ;
  ...
  function initUpdateCollection ( $Course , $IDMuser )
  {
    ...
    {                                                                           #  echo "Semesterapparat existiert schon"; 
      $IC[ 'title' ] = $Course[ 'fullname' ] ;
      $IC[ 'location_id' ] = $this -> resolveLocationID ( $IDMuser[ 'department' ] ) ;
      #$IC[ 'categories_id' ] = $IDMuser[ 'department' ] ;
      $IC[ 'title_short' ] = $Course[ 'shortname' ] ;
  ...
  function sendAdminEmails()
  {
    ...
    { 
      $message ="";
      $message .= "Liebe ELSE/HIBS Mitarbeiterin  \r\n\r\n";
      if ( $mi[9] > 0 OR $mi[1] > 0 )
      { 
        {                    $subject  = 'ELSE: Statusbericht -- '.$mi['id'] . " -- [ N:".$mi[1]." ] [ K:".$mi[9]." ]";
        ...
        $message .= "\r\n\r\nIhr ELSE Server \r\n\r\n";
        $message .= "http://www.elearning.haw-hamburg.de/mod/else/view.php?id=443297  \r\n\r\n";
