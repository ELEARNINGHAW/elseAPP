...
{if    $work.mode == "new"}
Neuen Semesterapparat anlegen für: {$colData[$work.collection_id].title}
{else}
{$colData[$work.collection_id].title}: Metadaten bearbeiten
{/if}
<a style="float:right;" href="index.php?item=collection&action=show&collection_id={$coll.title_short}&r={$user.role}"><img  class="icon" style="margin-top:-4px;" title="Zurück" src="img/svg/chevron-left_w.svg" /></a>
</h3>
...
<input type="hidden" name="action"         value="b_new_init">
{else}
<input type="hidden" name="action"         value="b_coll_meta_save">
...
<td> {html_options name="department_id" options=$tpl.departments selected=$colData[$work.collection_id].department }</td>
</tr>
</tbody>
</table>
