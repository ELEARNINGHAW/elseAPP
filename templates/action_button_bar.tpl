...
{/if}
{if $visible == 1}
<a class="icon" href="action.php?collection_id={$collection_id}&item={$item}&action={$action.button}&document_id={$document_id}"><img  class="icon" title="{$action.button_label}" src="img/svg/{$action.button}.svg" /></a>
{/if}{/foreach} 